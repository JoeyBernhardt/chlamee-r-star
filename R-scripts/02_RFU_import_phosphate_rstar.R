...
write_csv(plate_info, "data-processed/chlamee-phosphate-rstar-plate-info.csv")


RFU_files <- c(list.files("data-raw/phosphate-rstar-rfu", full.names = TRUE))
...
# 
# all_rfus3 %>% 
# 	ggplot(aes(x = population, y = RFU, color = factor(phosphate_level))) + geom_jitter(size = 3) +
# 	# geom_hline(yintercept = 7.889) +
# 	theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
# 	scale_color_viridis_d(name = "phosphate level")
# 
# ggsave("figures/phosphate-rstar-inoculation-rfus.pdf", width = 10, height = 6)


write_csv(all_rfus3, "data-processed/phosphate-rstar-rfus-time.csv")

all_rfus3 %>% 
	# filter(nitrate_level < 4) %>% 
	ggplot(aes(x = days, y = RFU, color = factor(phosphate_level), group = well_plate)) + geom_point() +
	geom_line() + facet_wrap(phosphate_level ~ population, scales = "free_y") + scale_color_viridis_d(name = "Phosphate level")
ggsave("figures/phosphate-rstar-rfus-time.pdf", width = 25, height = 20)

all_rfus3 %>% 
	ggplot(aes(x = days, y = RFU, color = factor(phosphate_level), group = well_plate)) + geom_point() +
	geom_line() + facet_wrap( ~ phosphate_level) + scale_color_viridis_d(name = "Phosphate level")
ggsave("figures/phosphate-rstar-rfus-time-phosphate-level.pdf", width = 15, height = 10)
